...
class DefaultHttpClientPropsValidationTest {
	...
	void shouldSucceedWithHttpVersion2() {
		contextRunner
				.withPropertyValues("tesseract.main.client.http-version=2")
				.run(context -> assertThat(context).hasNotFailed());
	}
	...
	void shouldFailWhenHttpVersionIsBelowMinimum() {
		contextRunner
				.withPropertyValues("tesseract.main.client.http-version=0")
	...
	void shouldFailWhenHttpVersionIsAboveMaximum() {
		contextRunner
				.withPropertyValues("tesseract.main.client.http-version=4")
				.run(context -> assertThat(context).hasFailed());
	}
	...
	void shouldSucceedWithValidConnectTimeout() {
		contextRunner
				.withPropertyValues("tesseract.main.client.connect-timeout=30s")
				.run(context -> assertThat(context).hasNotFailed());
	}
	...
	void shouldSucceedWithValidReadTimeout() {
		contextRunner
				.withPropertyValues("tesseract.main.client.read-timeout=30s")
				.run(context -> assertThat(context).hasNotFailed());
	}
